...
def gen_raw_events(config):
    ...
    def gen_raw_event(name, path, raw_event_config):
        ...
        if event_info.fail_reason_enum is None:
            parent_class = 'RawEvent'
        else:
            parent_class = 'RawFailableEvent<' + event_info.fail_reason_enum + '>'

        h_source = AutoGeneratedSource(path, class_name, 'h', CODE_GEN_CFG)

        with h_source.struct_block(class_name, inherits=[parent_class]):
            pass
