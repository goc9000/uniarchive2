...
class AutoGeneratedSource:
    ...
    def enum_class_block(self, name):
        return self._generic_block('enum class ' + name, semicolon=True)
    ...
    def struct_block(self, name, inherits=None):
        ...
        head = 'struct ' + name
        if len(inherits) > 0:
            head += ' : ' + ', '.join(inherits)  # TODO: maybe split this over multiple lines

        return self._generic_block(head, semicolon=True)
    ...
    @contextmanager
    def _generic_block(self, head, semicolon=False, nl_after=True):
    ...
    def switch_block(self, switch_by):
        return self._generic_block('switch (' + switch_by + ')')
    ...
    def _generic_head(self, head, tail, items, separator=','):
        one_line = head + (separator+' ').join(items) + tail
        ...
        for index, item in enumerate(items):
            self.line(item + (separator if index < len(items) - 1 else ''))

        self.unindent().line(tail)
    ...
    def declare_fn(self, function_name, return_type, *params, const=False, virtual=False, abstract=False):
        ...
        head = ('virtual ' if virtual else '') + return_type + ' ' + function_name + '('
        tail = ')' + (' const' if const else '') + (' = 0' if abstract else '') + ';'

        return self._generic_head(head, tail, [type + ' ' + name for type, name in params])

    def declare_constructor(self, class_name, *params):
        for type, _ in params:
            self.add_includes_for_type(type)

        return self._generic_head(class_name + '(', ');', [type + ' ' + name for type, name in params])
