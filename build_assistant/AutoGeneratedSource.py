    ...
    def add_includes_for_type(self, type):
        for part in re.split(r'[^A-Za-z0-9_]+', type):
            ...
            if symbol_info.include is not None:
                if isinstance(symbol_info.include, tuple):
                    kind, include = symbol_info.include
                    if kind == 'qt':
                    ...
                    self.include(symbol_info.include)

            if symbol_info.use is not None:
                if isinstance(symbol_info.use, tuple):
                    kind, use = symbol_info.use
                    if kind == 'std':
                        self.std_use(use)
                    else:
                        assert False, 'Use std'
                else:
                    self.use(symbol_info.use)
